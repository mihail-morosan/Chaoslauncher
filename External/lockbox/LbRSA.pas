...
type
  TLbRSA = class(TLbAsymmetricCipher)
    ...
    public {methods}
      ...
      procedure GenerateKeyPair(Generator:TLbRandomGenerator); override;
...
procedure GenerateRSAKeysEx(var PrivateKey, PublicKey : TLbRSAKey;
            ...
            Generator:TLbRandomGenerator);
procedure GenerateRSAKeys(var PrivateKey, PublicKey : TLbRSAKey;Generator:TLbRandomGenerator);
...
begin
  GenerateRSAKeysEx(PrivateKey, PublicKey, cLbDefAsymKeySize, cDefIterations, nil,Generator);
end;
{ -------------------------------------------------------------------------- }
procedure GenerateRSAKeysEx(var PrivateKey, PublicKey : TLbRSAKey;
...
begin
  ...
  try
    ...
    repeat
      { p , q = random primes }
      repeat
        p.RandomPrime(PrimeTestIterations,Generator);
        { check for abort }
        if Assigned(Callback) then
          Callback(Abort);
        if Abort then
          Exit;
        q.RandomPrime(PrimeTestIterations,Generator);
        { check for abort }
        if Assigned(Callback) then
          Callback(Abort);
        if Abort then
...
{ -------------------------------------------------------------------------- }
procedure TLbRSA.GenerateKeyPair(Generator:TLbRandomGenerator);
...
begin
  ...
  try
    GenerateRSAKeysEx(FPrivateKey, FPublicKey, FKeySize,
      FPrimeTestIterations, RSACallback,Generator);
  except
...
{ -------------------------------------------------------------------------- }
procedure TLbRSASSA.GenerateKeyPair(Generator:TLbRandomGenerator);
  { generate RSA public/private key pair }
begin
  if Assigned(FPrivateKey) then
    FPrivateKey.Free;
